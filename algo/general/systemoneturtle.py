...
class SystemOneTurtle:
    def strategy(self, df, symbol):
        df["10-Day High"] = df["Close"].rolling(window=10).max()
        df["10-Day Low"] = df["Close"].rolling(window=10).min()
    ...
    def condition(self, row):
        if row["Close"] > row["10-Day High"]:
            return 1
        elif row["Close"] < row["10-Day Low"]:
            return 0
        else:
            return 'No condition'
