...
class ThreeAndSixEma:
    def strategy(self, df):
        df["EMA6"] = ta.trend.EMAIndicator(close=df["Close"],
                                           window=6,
                                           fillna=False).ema_indicator()
        df["EMA3"] = ta.trend.EMAIndicator(close=df["Close"],
                                           window=3,
                                           fillna=False).ema_indicator()
        df["ATR10"] = ta.volatility.AverageTrueRange(
            high=df["High"], low=df["Low"],
            close=df["Close"], window=10, fillna=False).average_true_range()
        df["ADX10"] = ta.trend.ADXIndicator(high=df["High"], low=df["Low"],
                                            close=df["Close"], window=10, fillna=False).adx()
        df["Signal"] = df.apply(self.condition, axis=1)
        return df

    def condition(self, row):
        if row["EMA3"] > row["EMA6"] and row["ADX10"] > 25:
            return 1
