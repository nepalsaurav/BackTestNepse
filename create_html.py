...
def CreateHtml(strategy):
    ...
    total_count = (ms_df["Return Percent"] != 0).sum()
    summary_mcs += f"""
    <br>
    <h1>Probability Of Getting Positive Return: { round((ms_df["Return Percent"] > 0).sum()/total_count * 100, 2)}%</h1>
    <h1>Probability Of Getting Negative Return: { round((ms_df["Return Percent"] < 0).sum()/total_count * 100,2) }%</h1>
    """
